package PEReadUploader::PEReadUploaderClient;
...
PEReadUploader::PEReadUploaderClient
...
sub new
{
    ...
    my $self = {
	client => PEReadUploader::PEReadUploaderClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "PEReadUploaderTest.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for PEReadUploader::PEReadUploaderClient\n";
    }
    if ($sMajor == 0) {
        warn "PEReadUploader::PEReadUploaderClient version is $svr_version. API subject to change.\n";
    }
}
...
package PEReadUploader::PEReadUploaderClient::RpcClient;
